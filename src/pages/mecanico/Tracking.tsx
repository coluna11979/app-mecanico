  ...
  return (
    <div className="dark fixed inset-0 bg-steel-900 text-steel-100 flex flex-col">
      <div className="absolute top-3 left-3 right-3 z-10 flex justify-between items-start">
        <button onClick={() => nav('/mecanico/dashboard')} className="bg-steel-900/90 backdrop-blur rounded-full px-4 py-2 text-sm font-semibold">← Sair</button>
      ...
      <div className="bg-steel-800 border-t border-steel-700 p-5 animate-slide-up">
        {shop && (
          <>
            <div className="text-xs text-brand-500 uppercase tracking-wider font-bold">{job?.status === 'in_progress' ? 'Em serviço' : 'Indo para'}</div>
